...
military <- read_xlsx(here::here("week4", "practice_activities","data", "gov_spending_per_capita.xlsx"), 
...
```{r}
military_clean <- replace(military,military == "xxx" | military == ". .", NA)
...
military_clean <- military_clean |>
  mutate(
    across(
      .cols = c("1988":"2019"), .fns = as.numeric
    )
  )
...
anti_join(military_clean, cont_region, join_by(Country == Region))

```

## Part One Answer

What regions were not removed from the `military_clean` dataset? Use their correct capitalization **and** punctuation! Central America and the Carribean, Asia & Oceania, Middle East, Sub-Saharan
...
```{r}
ggplot(data = military_clean, mapping = aes(x = Spending, y = Year)) +
  geom_point()
